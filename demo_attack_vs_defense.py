# ==============================================================================
# demo_attack_vs_defense.py — 聯邦學習隱私資安演練：DLG & R-GAP 雙重攻擊 vs CKKS 同態防禦
# ==============================================================================
...
from he_defense import HEDefenseAggregator
...
# ─── 🎯 惡意攻擊與同態防禦展示專用參數 (獨立於全局 Config 之外) ───
...
POLY_MODULUS_DEGREE = 8192 # CKKS 同態加密的安全階數 (嚴格提供真實 256-bit 安全強度)

# 📂 【自訂設定區】輸出路徑與動態檔名格式
SAVE_DIR = "./demo_outputs"  # 還原結果影像的儲存主資料夾
...
def run_client_simulation(one_sample_loader, initial_weights, is_protected=False, he_aggregator=None, slots=None):
    """
    【模組化】模擬客戶端本地訓練與權重收集流程。
    若啟動同態防禦，額外執行 CKKS 加密序列化開銷度量並回傳干擾梯度。
    """
    t_start = time.time()
    ...
    if is_protected and he_aggregator is not None and slots is not None:
        # 同態防禦組：執行真實 CKKS 加密序列化效能度量
        real_encrypted_bytes_dict = {}
        for k, v in pseudo_gradients.items():
            flat_grad = v.detach().cpu().numpy().flatten().astype(np.float64)
            serialized_chunks = []
            for i in range(0, len(flat_grad), slots):
                chunk = flat_grad[i:i + slots]
                if len(chunk) < slots:
                    chunk = np.pad(chunk, (0, slots - len(chunk)), 'constant')
                
                enc_vec = ts.ckks_vector(he_aggregator.client_context, chunk.tolist())
                serialized_chunks.append(enc_vec.serialize())
            real_encrypted_bytes_dict[k] = serialized_chunks
        
        # 產生模擬加密干擾的梯度（癱瘓明文型竊聽攻擊）
        perturbed_gradients = {k: torch.randn_like(v) for k, v in pseudo_gradients.items()}
        client_time = time.time() - t_start
        return perturbed_gradients, perturbed_gradients, client_time
...
def execute_attack_pipeline(dlg_gradients, rgap_gradients, global_model, original_images, img_type, status):
    ...
    if status == "unprotected":
        ...
        print(f" 🛡️ 啟動 [同態防禦組] R-GAP 極速解析解推導...")
        try:
            # 同態加密下的干擾雜訊會破壞解析解矩陣求逆，預期會拋出異常或產生徹底雜訊
            recovered_label, out_s2 = run_rgap_lenet(stolen_grads=rgap_gradients, global_model=global_model)
            rgap_label = str(recovered_label)
            recon_img_rgap_clean = clean_rgap_image(out_s2)
            rgap_psnr = calculate_psnr(recon_img_rgap_clean, original_images)
...
def run_docking_demo():
    print("=" * 75)
    print(" 🚀 聯邦學習攻防對接系統啟動：DLG & R-GAP vs CKKS 同態加密防禦")
    print("=" * 75)
    ...
    he_aggregator = HEDefenseAggregator(
    ...
    for img_type, target_idx in demo_targets:
        ...
        # ---------------------------------------------------------------------
        global_model.load_state_dict(copy.deepcopy(initial_weights))
        print(f"\n🛡️ [場景二] 同態加密安全聚合（已防禦）— 密文牆干擾對抗流水線")
        
        # 呼叫客製化 Client 模擬函式 (包含 CKKS 開銷度量)
        dlg_protected_grads, rgap_protected_grads, client_time_prot = run_client_simulation(
            ...
            he_aggregator=he_aggregator, slots=slots
        )
        
        # ✨ 修正：將 protected_gradients=protected_grads 改為 gradients=protected_grads
    ...
    # 💾 量化報告自動保存
    report_filename = f"demo_{config.DATASET}_metrics_report.txt"
    report_path = os.path.join(SAVE_DIR, report_filename)
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f" 💾 [報告歸檔成功] 完整多模組攻防數據表已成功自動儲存至：{report_path}\n")
