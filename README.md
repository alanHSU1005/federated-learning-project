...
├── he_defense.py       # 同態加密防禦模組（基於 TenSEAL 實作真實私鑰隔離與密文盲算安全聚合）
...
- **SIMD 密文向量打包**：將訓練後的權重張量（Tensor）拉平，利用 SIMD 密集打包技術將數據分塊塞入 4096 個 Slots（當 Degree=8192 時），大幅降低同態運算與密文傳輸的時空開銷。
- **精密裁切對齊機制**：解密端內建動態裁切（Slicing），自動切除 SIMD 末端因長度不足而補上的零，確保還原後的張量與 PyTorch 模型維度完美重塑對齊。
- **自動誤差精確量化**：外層預先一次性計算明文 FedAvg 基準線，在密文聚合解密後，自動比對並即時列印每一層神經網路的 最大絕對解密誤差 (Max Decryption Error)，確保數值穩定性（控制在 $\le 10^{-5}$ 級別）。
...
當主流程掛載 HEDefenseAggregator 後，每輪通訊流程演變為：
1. Server 廣播全局權重 $\rightarrow$ 各 Client 在本地以明文訓練 $E$ 個 Epoch。
2. 【階段 1 — 客戶端加密】 Client 將訓練後權重展平打包，使用含私鑰的 client_context 進行 CKKS 同態加密，並立即序列化為純位元組（Bytes）發送給伺服器，斬斷私鑰參照。
3. 【階段 2 — 伺服器同態聚合】 Server 接收位元組流，強制使用不含私鑰的 server_context 反序列化為密文向量，在完全盲算狀態下進行加權聚合（乘上 weight_factors 並相加）。
4. 【階段 3 — 客戶端解密還原】 聚合密文模擬傳回安全端，使用私鑰解密、精確裁切、重塑為 PyTorch Tensor 後載入全局模型。
5. 精確度與效能監控：終端機將即時印出每層的解密誤差與通訊耗時度量
