...
def run_single_experiment(
    ...
    # ─── 🛡️ 以下為同態加密防禦模組動態注入點 (Step 3-1) ───
    if hasattr(config, 'ENABLE_HE_DEFENSE') and config.ENABLE_HE_DEFENSE:
        from he_defense import HEDefenseAggregator
        # 實例化安全聚合器，並傳入全局模型控制權
        he_aggregator = HEDefenseAggregator(
            server_model=server.get_global_model(), 
            poly_modulus_degree=config.HE_POLY_MODULUS_DEGREE
        )
        # 用同態聚合邏輯完全覆蓋並覆寫原本的明文 FedAvg 參數
        aggregation_fn = he_aggregator.aggregate
    # ───────────────────────────────────────────────────
    # ─── 🛡️ 以下為local dp 注入點 (Step 3) ───
    if hasattr(config, 'ENABLE_DP_DEFENSE') and config.ENABLE_DP_DEFENSE:
        from dp_defense import fl_dp_defense_hook
        dp_hook = lambda model: fl_dp_defense_hook(
            model=model, 
            max_norm=0.5, 
            noise_scale=0.05, 
            device=config.DEVICE
        )
        gradient_hook = dp_hook
    # -------------------------------------------------------------------------
...
def run_single() -> None:
    ...
    from dp_defense import fl_dp_defense_hook
        ...
        # 定義本次實驗的 DP 參數
        # 可以先從 max_norm=0.5, noise_scale=0.01 開始調校
    dp_hook = lambda model: fl_dp_defense_hook(
        model=model, 
        max_norm=0.5, 
        noise_scale=0.05, 
